...
def getPParray(pointing, time, position, height_array):
    ...
    pp, am = PosTools.getPPsimpleAngle(
        height=height_array, mPosition=position, direction=itrfdir
    )
    return pp, am
...
def getBarray(pointing, time, position, height_array):
    ...
    emm = EMM.WMM(date=year + float(dayofyear) / 365.0)
    BField = emm.getProjectedFieldArray(
        np.degrees(pp[:, 0]), np.degrees(pp[:, 1]), pp[:, 2] / 1e3, los_dir
    )
    return pp, am, BField
...
def getTECarray(
    ...
    ionexf = ionex.getIONEXfile(
        time=date_parms, server=server, prefix=prefix, outpath=ionexPath
    )
...
def getRM_raytrace(time, pointing, position):
    ...
    for lon, lat, h in zip(
        np.degrees(pp[:, 0]), np.degrees(pp[:, 1]), pp[:, 2] / 1.0e3
    ):
        myiri.lon = lon
        myiri.lat = lat
