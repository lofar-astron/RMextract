...
def init_Meerkat(
    src="00:34:08.8703 -07:21:53.409",
    times=[58774.9430718395, 59034.0376808637, 59064.9741027126, 59090.8953866498],
):
    ...
    statposarray = np.array(
        [[statpos.x.value, statpos.y.value, statpos.z.value]] * len(times)
    )
    srcpos = SkyCoord(src, unit=(u.hourangle, u.deg), frame="fk5")
    atimes = [Time(i, format="mjd") for i in times]

    altazdir = [
        srcpos.transform_to(AltAz(obstime=itime, location=statpos)) for itime in atimes
    ]
...
def init_from_file(fname, src="05:28:52.264 +22:00:04"):
    ...
    statpos = [
        EarthLocation.from_geocentric(*mydb.phase_centres[i], unit=u.m) for i in statpos
    ]
    statposarray = np.array([[i.x.value, i.y.value, i.z.value] for i in statpos])
    srcpos = SkyCoord(src, unit=(u.hourangle, u.deg), frame="fk5")
    atimes = Time(data[:, 0], format="mjd")
    altazdir = [
        srcpos.transform_to(AltAz(obstime=itime, location=pos))
        for pos, itime in zip(statpos, atimes)
    ]
    itrfdir = [i.transform_to("itrs") for i in altazdir]
    direction = np.array([[i.x, i.y, i.z] for i in itrfdir]).T
    return statposarray, srcpos, direction, atimes
...
def init_from_time_file(fname, src="10:22:57.9992 +10:01:52.78"):
    ...
    statpos = ["CS002LBA" if i == "LOFAR" else i + "HBA" for i in stations] * tms.shape[
        0
    ]
    statpos = [
        EarthLocation.from_geocentric(*mydb.phase_centres[i], unit=u.m) for i in statpos
...
def init_from_nat_file(
    fname, src="10:22:57.9992 +10:01:52.78", skiprows=0, stations=None
):
...
idx_hbot = h.shape[0]
h = np.concatenate(
    (h, np.arange(h[-1] + hstep2 * 1e3, hend * 1e3 + 0.5 * hstep2 * 1e3, hstep2 * 1e3))
)
hweights = np.concatenate(
    (hstep1 * np.ones(idx_hbot), hstep2 * np.ones(h.shape[0] - idx_hbot))
)
...
def getRM_iri(
    ...
    for itime, (time, statpos) in enumerate(zip(times, statposarray)):
        ...
        print(itime, time)
        pp, am = pt.getPPsimpleAngle(
            height=h, mPosition=statpos, direction=directions[:, itime]
        )
        try:
        ...
        vTEC = ionex.getTECinterpol(
            time=np.resize((time.mjd - int(time.mjd)) * 24, latpp.shape),
            lat=latpp,
            lon=lonpp,
            tecinfo=tecinfo,
        ...
        emm = EMM.WMM(
            date=time.decimalyear, lon=lonpp[hidx], lat=latpp[hidx], h=h[hidx] * 1e-3
        )

        BField = emm.getProjectedFieldArray(
            lonpp, latpp, h * 1e-3, los_dir=[londir[itime], latdir[itime]]
        )
        Bpars.append(BField)
        ...
        myiri = pyiriplas.pyiriplas(
            year=time.ymdhms.year,
            month=time.ymdhms.month,
            day=time.ymdhms.day,
            hour=time.ymdhms.hour + time.ymdhms.minute / 60.0,
        )
        ...
        plasprofiles.append(
            np.concatenate(
                (
                    myiri.get_profile(h[:idx_hbot] * 1e-3),
                    myiri.get_profile(h[idx_hbot:] * 1e-3),
                )
            )
        )
    ...
    ntop = (
        topprofiles
        / np.sum(np.diagonal(topprofiles, 0, 1, 2), axis=1)[:, np.newaxis, np.newaxis]
    )
    finalp = []
...
def getRM_iri_new(
    ...
    for itime, (time, statpos) in enumerate(zip(times, statposarray)):
        ...
        BField = emm.getProjectedFieldArray(
            lonpp, latpp, h * 1e-3, los_dir=[londir, latdir]
        )
        Bpars.append(BField)
        #    myiri=pyiri.pyiri(year=time.ymdhms.year,month=time.ymdhms.month,day=time.ymdhms.day,hour=time.ymdhms.hour + time.ymdhms.minute/60.)
