...
def ITRFToWGS84(x, y, z):
    ...
    p = sqrt(x * x + y * y)
    q = atan2(z * sm_a, (p * b))
    lat = atan2((z + E * b * sin(q) * sin(q) * sin(q)), (p - e2 * sm_a * cos(q) * cos(q) * cos(q)))
...
def getlonlatheight(az,el,position,h=ION_HEIGHT):
    if HAS_PYRAP:
      ...
      lat,lon,ht = aer2ecef(degrees(az), degrees(el), slantRange, location_lat, location_lon, ION_HEIGHT)
      # convert to itrf coordinates on sphere with radius 1
      lat = radians(lat)
      lon = radians(lon)
      diritrf=[cos(lat)*cos(lon),cos(lat)*sin(lon),sin(lat)]
...
def getAzEl(pointing,time,position,ha_limit=-1000):
    ...
    if HAS_PYRAP:
        ...
        equatorial = ephem.Equatorial(str(12.0 * degrees(pointing[0])/180),str(degrees(pointing[1])))
