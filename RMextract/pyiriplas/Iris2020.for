C IRIS2020.FOR_____________________________ Oct.2020 T.L.Gulyaeva
...
C
      SUBROUTINE IRIS2020(JF,IYYYY,MMDD,DHOUR,HEIBEG,HEIEND,NUMHEI
     &    ,OUTF,JIND,LATI,ALONGI,MLAT,MLONG,MODIP,DIP,MAGBR)                           
C-----------------------------------------------------------------
C
...
C*****************************************************************
     ...
     &          /CONST1/HUMR,DUMR,XFAP,INDAP,AKP,COV,RZS1,RZS2
...
c      write(25,*) '*** ISO_IRI parameters are being calculated ***'
      if(NODEN) goto 2889
c         if(LAYVER) write(*,*) 'Ne, E-F: The LAY-Version is ',
c     &      'prelimenary. Erroneous profile features can occur.'
c         if(GULB0) write(25,*) 'Ne, B0: Bottomside thickness is ',
c     &      'obtained with Gulyaeva-1987 model.'
      if(tecin) then 
      write(25,*) 'TEC: input values are used.'
      write(25,*) '    hmF2: CCIR model is fitted to TEC.' 
      write(25,*) 'Ne, foF2: CCIR model is fitted to TEC.'
      goto 8201
      endif
         if(OLD79) write(*,*) 'Ne: Using IRI-79. Correction',
     &      ' of equatorial topside is not included.'
      if (.not.(HMF2IN)) THEN
      write(25,*) '    hmF2: CCIR model is used.' 
       goto 700
                      ELSE
      if(hmf2.GT.0.) then
        write(25,*) 'hmF2: Input values are used.'
      else
        write(25,*) 'M3000F2: Input values are used.'
      endif
                      ENDIF
 700  continue
         if(FOF2IN) then
          write(25,*) 'Ne, foF2: Input values are used.'
            goto 2889
            endif
         if(URSIF2) then
          write(25,*) 'Ne, foF2: URSI model is used.'
         else
          write(25,*) 'Ne, foF2: CCIR model is used.'
         endif
      if(jf(26)) then
       write(25,*) 'Ne, foF2:  STORM  model included.'
       write(25,*) 'hmF2(foF2) STORM2 model included.'
      endif
2889     continue
      if((.not.NOION).and.(DY))
...
C
C_ADD
C/      call igrf_dip(lati,ylongi,ryear,300.0,dec,dip,magbr,modip) ! NEW IGRF2015
C/        IF (MAGBR.GT.9998.) THEN          
C/        CALL FIELDG(LATI,LONGI,300.0,XMA,YMA,ZMA,BET,DIP,DEC,MODIP) !OLD POGO-75
C/        MAGBR=ATAN(0.5*TAN(DIP*UMR))/UMR
C/        ENDIF
C_END_ADD
        ABSLAT=ABS(LATI)       
        ABSMLT=ABS(MLAT)
...
2619  HOUR=DHOUR
      ...
      idayinp=iday           
                        call MODA(1,iyear,MONTH,IDAY,DAYNR,nrdaym)
C 
      if (iday.ne.idayinp) mmdd=month*100+iday  
                        endif
c
...
C NEW TLG: the following statements commented <! gec_rz.dat> are using gec_rz.dat file:
      IF (jind.eq.0) THEN                    
      call tcongec(iyear,month,iday,daynr,rzar,arig,ttt,nmonth)   
                     ELSE                     
...
Crem         W=RSSN
      ...
      EH=20200.
      hprr=cos(mlat*umr)
      HHH=((5.7*hprr*hprr-1.)*6370.+500.)*(1.-0.0825*XKP)    
      HPL=20200.  
      heiend=hpl
C
...
C
      dlogne=2.0*alog10(ratfc)  
      call subdlogh(dlogne,mlat,sday,rzs,dlgh) 
      rathhm=10.**dlgh 
...
C# NEW!!! T.L.Gulyaeva CORRECTION OF BENT MODEL FOR h05top HEIGHT
      hei05=0.
C-         CALL TOPH05(RSSN,MLAT,HOUR,HMF2,HEI05,sday)
         CALL TOPH05(RZS1,MLAT,HOUR,HMF2,HEI05,sday)
c
      h05top=hei05
...
C-      B0 = B0_98(HOUR,SAX,SUX,NSEASN,RSSN,YLONGI,MODIP) ! IRI-2001
      B0 = B0_98(HOUR,SAX,SUX,NSEASN,RZS1,YLONGI,MODIP) ! IRI-2001
        endif
      HHALF = GRAT * HMF2
C!!!!!!! F1-REGION PARAMETERS !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C1=0.
      IF(NIGHT) GOTO 150
C-        FOF1=FOF1ED(ABSMBR,RSSN,XHI)
        FOF1=FOF1ED(ABSMBR,RZS1,XHI)
...
c
C-2727  NMD=XMDED(XHI,RSSN,4.0E8)
2727  NMD=XMDED(XHI,RZS1,4.0E8)
...
C
      dlogne=alog10(xnef)-alog10(xnefm)  
      call subdlogh(dlogne,mlat,sday,rzs,dlgh) 
      rathhm=10.**dlgh !new
      if (rathhm.lt.0.8) rathhm=0.8
...
C Check Ti(h<400km) for interpolation:
      ...
      kkti=kk-2
	if(kkti.lt.0) kkti=0	  ! Corr. 01.09.2021
