C IRIF2020.FOR________________________Apr 2020 (T.L.Gulyaeva)
C
...
C MIDNIGHT (TE(5)) AND NOON (TE(6)).
     ...
     &.413E-1,.4876E-2,.2692E-1,.1684E-2/
      DATA C(1,2,:)/.313654E1,.6796E-2,.181413,.8564E-1,
     ...
     &-.6992E-3,-.115E-1,-.1644E-2,.3355E-2,-.4326E-2,.2035E-1,.2985E-1/
      DATA C(2,1,:)/.3136E1,.6498E-2,.2289,.1859E-1,-.3328E-1,
     ...
     &-.854E-3,-.1126E-1,-.2323E-2,-.8582E-2,.2683E-1/
      DATA C(2,2,:)/.3144E1,.8571E-2,.2539,.6937E-1,-.1667E-1,
     ...
     &.6405E-3,.5977E-2,.1333E-1/
      DATA C(3,1,:)/.3372E1,.1006E-1,.1436,.2023E-2,-.5166E-1,
     ...
     &-.2425E-2,.1583E-1,.9577E-2/
      DATA C(3,2,:)/.3367E1,.1038E-1,.1407,.3622E-1,-.3144E-1,
     ...
     &-.5005E-2,-.115E-1,.2574E-1/
      DATA C(4,1,:)/.3574E1,.0,.7537E-1,.0,-.8459E-1,
     ...
     &.0,-.251E-2,.2434E-1/
      DATA C(4,2,:)/.3574E1,-.5639E-2,.7094E-1,
...
c predictions.
      if(iflag.eq.0) then
         open(unit=12,file=TRIM(ADJUSTL(DATAPATH))//'gec_rz.dat',ERR=10,
     &    status='old')
c-web- special for web version
c          open(unit=12,file=
...
c----------------------------------------------------------------
C  TL Gulyaeva........................................................ Apr, 2020
C     nind =1 <DEFAULT> ssn1_12.dat  =>  rz(3); F10.7  =>  sf(3) 
C     nind =2 ssn2_12.dat  SSN2   =>  rz(3); F10.7  => sf(3)
...
C        
    2 xxpr='ssn2'      
      call subproxy(xxpr,yr,mm,iyy2,imm2,rz)  
Cno      rz(1)=0.7*rz(1)	! Keep rz(1)=SSN2
Cno      rz(2)=0.7*rz(2)	! Keep rz(2)=SSN2
      yypr='f107'
      call subproxy(yypr,yr,mm,iyy2,imm2,sf)  
      goto 33
C        
...
C
      ...
      zz(3)=0.0
C
      open(unit=12,file=TRIM(ADJUSTL(DATAPATH))//infile,ERR=10,
     &  status='old')
      GOTO 20
...
C **********************************************************************
      DIMENSION         PLG(9,4)
     *,indap(13),akp(13)
        common  /const/umr,PI /const1/hr,dr,xfap,indap,akp,cov,RZS1,RZS2
...
!Copyright: John Titheridge, Physics Department, University of Auckland, New Zealand.
      ...
      parameter Re=6371.2
      parameter ho=400
      parameter Ro=1+ho/Re
      parameter Ro2=Ro**2                !Refernce ht for Lat,Go,To
