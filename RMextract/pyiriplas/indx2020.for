C INDEX SUBROUTINES:___INDX2020.FOR___T.L.Gulyaeva___Oct. 2020  
C
...
C
      ...
      OPEN(13,FILE=infilekp,STATUS='OLD',ERR=21)  
  201 continue
                do jj=1,8
                        iap2(jj)=iap1(jj)
                        iap1(jj)=iap0(jj)
...
c
C------------------------------------------------------------------------
      subroutine subcoords(JMAG,IYEAR,LDA,ALATI,ALONG,AMLAT,AMLON,AMODIP
     &                       ,DIP,AMAGBR) 
C TLG.....................................................Oct. 2020
C
C  corvert geographic-to-geomagnetic and geomagnetic-to-geographic coordinates
C
C
...
C
      COMMON/IGRF1/     ERA,AQUAD,BQUAD,DIMO      !+2020
         COMMON /CONST/UMR,PI
C
       PI=ATAN(1.0)*4.
       UMR=PI/180.
...
C
       if(along.lt.0.0) along = along + 360.
        ...
        ryear = iyear + (lda-1.0)/idayy
C
      CALL GEODIP(IYEAR,ALATI,ALONG,AMLAT,AMLON,JMAG)
C
      CALL FELDCOF(RYEAR)                      
C_ADD
      call igrf_dip(alati,along,ryear,300.0,dec,dip,amagbr,amodip) 
        IF (AMAGBR.GT.9998.) THEN          
        CALL FIELDG(ALATI,ALONG,300.0,XMA,YMA,ZMA,BET,DIP,DEC,AMODIP) 
        AMAGBR=ATAN(0.5*TAN(DIP*UMR))/UMR
        ENDIF
C_END_ADD
	RETURN
	END
