...
def createRMh5parm(
    ...
    try:
        mslist = (
            MSfiles.lstrip("[").rstrip("]").replace(" ", "").replace("'", "").split(",")
        )
    ...
    if solset_name not in data.getSolsetNames():
    ...
    for time_array in times[
        ::-1
    ]:  # check in reverse order, since datamight not be available for latest days
        starttime = time_array[0]
        date_parms = PosTools.obtain_observation_year_month_day_fraction(starttime)
        if not proxyServer:
            if "http" not in ionex_server:  # ftp server use ftplib
        ...
        if ionexf == -1:
            if "igsiono.uwm.edu.pl" not in ionex_server:
    ...
    if all_stations:
        ...
        rm_vals = np.ones(
            (len(station_names), rmdict["RM"]["st0"].shape[0]), dtype=float
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Adds CommonRotationAngle to an H5parm from RMextract."
    )
    ...
    parser.add_argument(
        "--single",
        "-s",
        help="calculate RM only for CS002LBA (default calculate for all stations in the MS)",
        action="store_false",
        dest="allStations",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0,
        help="float parameter describing how much of Earth rotation is taken in to account \
    ...
    createRMh5parm(
        MS,
        h5parmdb,
        ionex_server=args.server,
        ionex_prefix=args.prefix,
        ionexPath=args.ionexpath,
        solset_name=args.solsetName,
        all_stations=args.allStations,
        timestepRM=args.timestep,
