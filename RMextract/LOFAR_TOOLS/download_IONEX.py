...
def main():
    parser = argparse.ArgumentParser(
        description="Downloads relevant IONEX files for a given MS"
    )
    parser.add_argument(
        "MSfiles", type=str, help="MS for which the parmdb should be created."
    )
    parser.add_argument(
        "--server",
        type=str,
    ...
    for time_array in times[
        ...
        if "http" not in server:  # ftp server use ftplib
            ionexf = ionex.getIONEXfile(
                time=date_parms, server=server, prefix=prefix, outpath=args.ionexpath
            )
        else:
        ...
        if ionexf == -1:
            if "igsiono.uwm.edu.pl" not in server:
