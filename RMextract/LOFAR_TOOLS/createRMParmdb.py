...
def createRMParmdb(
    ...
    if create:
        myParmdb.addDefValues("Gain:0:0:Ampl", 1.0e-4)
        myParmdb.addDefValues("DirectionalGain:0:0:Ampl", 1.0)
        myParmdb.addDefValues("DirectionalGain:1:1:Ampl", 1.0)
        myParmdb.addDefValues("Gain:0:0:Real", 1.0e-4)
        myParmdb.addDefValues("Gain:1:1:Real", 1.0e-4)
        myParmdb.addDefValues("DirectionalGain:0:0:Real", 1.0)
        myParmdb.addDefValues("DirectionalGain:1:1:Real", 1.0)
    myMS = tab.table(MS)
    ...
    for st in stations:
        ...
        myValue = myParmdb.makeValue(
            ...
            efreq=2e10,
            stime=result["times"],
            etime=np.ones(result["times"].shape, dtype=float) * result["timestep"],
            asStartEnd=False,
        )
        if patchname:
...
def main():
    descriptiontext = (
        "Create a parmDB with the Ionospheric RM predicted from data in IONEX files.\n"
    ...
    parser.add_argument(
        "MS", help="Measurement-Set for which the parmDB is to be created."
    )
    parser.add_argument(
        "-o", "--out", help="name of the parmdb", dest="parmdbName", default="RMParmdb"
    )
    parser.add_argument(
        "-p",
        "--patch",
    ...
    parser.add_argument(
        "--IONprefix",
        help="prefix of IONEX files, either CODG or ROBR",
        dest="prefix",
        default="codg",
    )
    parser.add_argument(
        "--IONserver",
        help="server of IONEX files",
        dest="server",
        default="ftp://cddis.gsfc.nasa.gov/gnss/products/ionex/",
    )
    parser.add_argument(
        "--IONpath", help="location of IONEX files", dest="ionexPath", default="./"
    )
    parser.add_argument(
        "--all",
        "-a",
        help="calculate RM per station (default calculates only for CS002LBA)",
        action="store_true",
        dest="allStations",
    )
    parser.add_argument(
        "-t",
        "--timestep",
        help="timestep in seconds. for values <=0 (default) the timegrid of the MS is used ",
        dest="timestep",
        type=float,
        default=900.0,
    )
    parser.add_argument(
        "-e",
        "--smart_interpol",
        help="float parameter describing how much of earth rotation is taken in to account in interpolation of the IONEX files. 1.0 means time interpolation assumes ionosphere rotates in opposite direction of the Earth. 0.0 (default) means no rotation applied",
        dest="earth_rot",
        type=float,
        default=0,
    )
    parser.add_argument(
        "-r",
        "--overwrite",
        help="overwrite existing parmdb",
        action="store_true",
        dest="overwrite",
    )
